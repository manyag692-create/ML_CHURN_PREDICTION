...
/* ===== NORMAL TEXT VISIBILITY (white & crisp) ===== */
...
.stApp a { color: #00F5FF !important; }
...
.success-box * { color: #000 !important; }
...
.danger-box * { color: #fff !important; }
...
# =========================================================
st.markdown("""
<div class="holographic-title">🚀 NEXT GEN AI CHURN SYSTEM</div>
<div class="hero-subtitle">✨ AI-Powered Customer Intelligence Dashboard ✨</div>
""", unsafe_allow_html=True)
...
# =========================================================
st.sidebar.markdown("""
<h2 style="font-family:'Orbitron',monospace;color:#00F5FF;text-align:center;">🎮 NAVIGATION</h2>
""", unsafe_allow_html=True)
...
# =========================================================
if menu == "🏠 Dashboard":
    st.markdown('<div class="section-header">🌌 Enterprise Intelligence Center</div>', unsafe_allow_html=True)
    ...
    with col1:
        ...
        fig_surface = go.Figure(data=[go.Surface(
            z=Z, x=X, y=Y, colorscale='Turbo', showscale=True,
            colorbar=dict(title=dict(text="Customer Value", font=dict(color='white')), tickfont=dict(color='white'))
        )])
        fig_surface.update_layout(
            title=dict(text="🌊 Customer Value Surface Analysis", font=dict(size=20, color='cyan', family='Orbitron')),
            scene=dict(
            ...
            paper_bgcolor='rgba(0,8,20,0.8)', font=dict(color='white'),
    ...
    with col2:
        ...
        fig_scatter3d = go.Figure(data=[go.Scatter3d(
            x=scatter_data['Credit_Score'], y=scatter_data['Age'], z=scatter_data['Balance'],
            mode='markers',
            marker=dict(size=8, color=scatter_data['Churned'],
        ...
        fig_scatter3d.update_layout(
            title=dict(text="🎯 Customer Segmentation 3D View", font=dict(size=20, color='cyan', family='Orbitron')),
            scene=dict(
    ...
    for i, (dept, color) in enumerate(zip(departments, colors)):
        fig_3dbar.add_trace(go.Scatter3d(
            x=months, y=[dept] * len(months), z=z_data[i],
            mode='markers+lines',
            marker=dict(size=12, color=color, symbol='diamond', line=dict(color='white', width=2)),
            line=dict(color=color, width=6), name=dept
        ))
    fig_3dbar.update_layout(
        title=dict(text="📊 3D Department Performance Matrix", font=dict(size=24, color='cyan', family='Orbitron')),
        scene=dict(
            ...
            bgcolor='rgba(0,0,0,0)', camera=dict(eye=dict(x=1.5, y=1.5, z=1))
        ),
        paper_bgcolor='rgba(0,8,20,0.8)', font=dict(color='white'),
        height=600, showlegend=True,
        legend=dict(bgcolor='rgba(0,0,0,0.5)', bordercolor='cyan', borderwidth=1)
    )
    st.plotly_chart(fig_3dbar, use_container_width=True)

elif menu == "📊 Analytics":
    st.markdown('<div class="section-header">📊 Advanced Business Analytics</div>', unsafe_allow_html=True)
    ...
    with col1:
        if len(numeric_df.columns) >= 2:
            ...
            hist_data = np.histogram2d(numeric_df[x_col].fillna(0), numeric_df[y_col].fillna(0), bins=20)
            fig_hist3d = go.Figure(data=[go.Surface(z=hist_data[0], colorscale='Viridis', showscale=True)])
            fig_hist3d.update_layout(
                title=dict(text="📈 3D Distribution Analysis", font=dict(size=20, color='cyan', family='Orbitron')),
                scene=dict(
                ...
                paper_bgcolor='rgba(0,8,20,0.8)', font=dict(color='white'), height=450
            )
    ...
    with col2:
        if len(numeric_df.columns) >= 1:
            ...
            fig_violin.add_trace(go.Violin(
                ...
                box_visible=True, line_color='cyan',
                fillcolor='rgba(0, 245, 255, 0.3)', opacity=0.8,
                meanline_visible=True, name=sample_col
            ))
            fig_violin.update_layout(
                title=dict(text="📦 Risk Distribution Analysis", font=dict(size=20, color='cyan', family='Orbitron')),
                paper_bgcolor='rgba(0,8,20,0.8)', plot_bgcolor='rgba(0,0,0,0)',
                font=dict(color='white'), height=450, yaxis=dict(gridcolor='rgba(255,255,255,0.1)')
            )
    ...
    fig_heatmap.update_layout(
        title=dict(text="🔥 3D Correlation Matrix", font=dict(size=24, color='cyan', family='Orbitron')),
        scene=dict(
        ...
        paper_bgcolor='rgba(0,8,20,0.8)', font=dict(color='white'), height=600
    )
    ...
    fig_timeline.add_trace(go.Scatter(
        x=timeline_data['Month'], y=timeline_data['Churn_Rate'],
        mode='lines+markers', name='Churn Rate %',
        line=dict(color='#ff1744', width=4), marker=dict(size=12, symbol='circle'),
        fill='tozeroy', fillcolor='rgba(255, 23, 68, 0.2)'
    ))
    fig_timeline.add_trace(go.Scatter(
        x=timeline_data['Month'], y=timeline_data['Satisfaction'],
        mode='lines+markers', name='Satisfaction %',
        line=dict(color='#00ff88', width=4), marker=dict(size=12, symbol='diamond'),
        fill='tozeroy', fillcolor='rgba(0, 255, 136, 0.2)'
    ))
    fig_timeline.update_layout(
        title=dict(text="📈 Annual Performance Timeline", font=dict(size=24, color='cyan', family='Orbitron')),
        paper_bgcolor='rgba(0,8,20,0.8)', plot_bgcolor='rgba(0,0,0,0)',
    ...
    st.markdown('<div class="section-header">👨‍💻 Elite Tech Team</div>', unsafe_allow_html=True)
    ...
    for i, row in dept_data.iterrows():
        fig_dept.add_trace(go.Scatter3d(
            x=[row['Department']], y=[row['Projects']], z=[row['Efficiency']],
            mode='markers',
            marker=dict(size=row['Team_Size'] * 3, color=row['Efficiency'],
                        colorscale='Turbo', opacity=0.9, line=dict(color='white', width=2)),
            name=row['Department'],
            text=f"{row['Department']}<br>Efficiency: {row['Efficiency']}%<br>Projects: {row['Projects']}<br>Team: {row['Team_Size']}",
            hoverinfo='text'
        ))
    fig_dept.update_layout(
        title=dict(text="🏢 3D Department Performance Visualization", font=dict(size=24, color='cyan', family='Orbitron')),
        scene=dict(
            ...
            bgcolor='rgba(0,0,0,0)', camera=dict(eye=dict(x=1.8, y=1.8, z=0.8))
        ),
        paper_bgcolor='rgba(0,8,20,0.8)', font=dict(color='white'),
    ...
    fig_radar.add_trace(go.Scatterpolar(
        r=[95, 88, 92, 90, 97, 93], theta=categories, fill='toself',
        fillcolor='rgba(0, 245, 255, 0.3)', line=dict(color='cyan', width=3),
        name='Team Expertise'
    ))
    fig_radar.add_trace(go.Scatterpolar(
        r=[85, 90, 88, 95, 82, 88], theta=categories, fill='toself',
        fillcolor='rgba(255, 0, 255, 0.3)', line=dict(color='magenta', width=3),
        name='Industry Standard'
    ))
    fig_radar.update_layout(
        title=dict(text="🎯 Team Skills Matrix", font=dict(size=24, color='cyan', family='Orbitron')),
        polar=dict(
            bgcolor='rgba(0,0,0,0)',
    ...
    if st.button("🚀 EXECUTE AI PREDICTION"):
        ...
        for i, stage in enumerate(stages):
            status_text.markdown(f"<h4 style='color:#00F5FF;text-align:center;'>{stage}</h4>", unsafe_allow_html=True)
            for j in range(20):
        ...
        if credit_score < 500: risk_factors += 0.15
        elif credit_score < 600: risk_factors += 0.08
        elif credit_score > 750: protective_factors += 0.12
        elif credit_score > 700: protective_factors += 0.06

        if age < 25: risk_factors += 0.05
        elif age > 50: protective_factors += 0.05

        if tenure <= 1: risk_factors += 0.12
        elif tenure >= 7: protective_factors += 0.15
        elif tenure >= 5: protective_factors += 0.08

        if balance < 1000: risk_factors += 0.08
        elif balance > 100000: protective_factors += 0.08

        if products == 1: risk_factors += 0.10
        elif products >= 3: protective_factors += 0.12
        elif products == 2: protective_factors += 0.05

        if active_val == 0: risk_factors += 0.18
        else: protective_factors += 0.15

        if card_val == 0: risk_factors += 0.05
        else: protective_factors += 0.03

        if salary > 100000: protective_factors += 0.05
        elif salary < 30000: risk_factors += 0.05
        ...
        prediction = 1 if churn_probability >= 0.55 else 0
        ...
        with res1:
            gauge_churn = go.Figure(go.Indicator(
                ...
                gauge={
                    ...
                    'threshold': {'line': {'color': 'cyan', 'width': 4}, 'thickness': 0.8, 'value': churn_probability * 100}
                }
            ))
            gauge_churn.update_layout(paper_bgcolor='rgba(0,8,20,0.8)', font={'color': 'white'}, height=350)
        ...
        with res2:
            gauge_retention = go.Figure(go.Indicator(
                ...
                gauge={
                    ...
                    'threshold': {'line': {'color': 'magenta', 'width': 4}, 'thickness': 0.8, 'value': retention_probability * 100}
                }
            ))
            gauge_retention.update_layout(paper_bgcolor='rgba(0,8,20,0.8)', font={'color': 'white'}, height=350)
        ...
        if prediction == 1:
            ...
            <div class="danger-box">
                ⚠️ HIGH CHURN RISK DETECTED<br><br>
                📉 {round(churn_probability * 100, 2)}%<br>
            ...
            st.markdown("""
            <div class="glass-card">
                <h3 style="color:#ff5252;">🚨 Risk Factor Analysis</h3>
                <ul>
            ...
            <div class="success-box">
                ✅ CUSTOMER RETAINED<br><br>
                💚 {round(retention_probability * 100, 2)}%<br>
            ...
            st.markdown("""
            <div class="glass-card">
                <h3 style="color:#00ff88;">💎 Loyalty Indicators</h3>
                <ul>
        ...
        fig_features.add_trace(go.Scatter3d(
            x=feature_importance['Feature'], y=feature_importance['Impact'], z=feature_importance['Your_Value'],
            mode='markers+text',
            marker=dict(size=feature_importance['Impact'] * 100, color=feature_importance['Your_Value'],
                        colorscale='RdYlGn', opacity=0.9, line=dict(color='white', width=2)),
            text=feature_importance['Feature'], textposition='top center',
            textfont=dict(size=10, color='white')
        ))
        fig_features.update_layout(
            title=dict(text="🎯 3D Feature Impact Matrix", font=dict(size=20, color='cyan', family='Orbitron')),
            scene=dict(
            ...
            paper_bgcolor='rgba(0,8,20,0.8)', font=dict(color='white'), height=500
        )
...
st.markdown("""
<div class="footer">
    <p>⚡ Powered By Streamlit • Plotly • Machine Learning • Advanced AI</p>
    <h4>🚀 Next Generation Customer Intelligence Platform v2.0</h4>
</div>
""", unsafe_allow_html=True)
